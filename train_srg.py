...
def train_srgan(generator, discriminator, dataloader, device, lr=1e-4, total_steps=2e5, display_step=500):
    ...
    g_optimizer = th.optim.Adam(generator.parameters(), lr=lr)
    d_optimizer = th.optim.Adam(discriminator.parameters(), lr=lr)
    g_scheduler = th.optim.lr_scheduler.LambdaLR(g_optimizer, lambda _: 0.1)
    d_scheduler = th.optim.lr_scheduler.LambdaLR(d_optimizer, lambda _: 0.1)
    ...
    while cur_step < total_steps:
        for hr_real, lr_real in tqdm(dataloader, position=0):
            ...
            # Enable autocast to FP16 tensors(new feature sicne torch==1.6.0)
            # If you're running older versions of torch, comment this out
            # and use NVIDIA apex for mixed/half percision training
            if has_autocast:
                with th.cuda.amp.autocast(enabled=(device=='cuda')):
                    g_loss, d_loss, hr_fake = loss.fn(
...
th.save(generator, 'srgenerator.pt')
th.save(discriminator, 'srdiscriminator.pt')
